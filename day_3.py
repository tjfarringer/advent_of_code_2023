    ...
    for line_idx, line in enumerate(data):
        ...
        while not first_number_indx_rng is None:
            ...
            surrounded = utilities.surrounded_by_symbol(
                data, line_idx, first_number_indx_rng)
            # 3 -- if so, store, otherwise move onto the next number
            if surrounded:
                print(
                    f'value: {data[line_idx][first_number_indx_rng[0]:first_number_indx_rng[1]]}')
                part_numbers.append(
                    int(data[line_idx][first_number_indx_rng[0]:first_number_indx_rng[1]]))

            search_range = first_number_indx_rng[1]+1
